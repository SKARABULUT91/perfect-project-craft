...
import { likeTweet, retweetTweet, followUser, postTweet, searchTweets } from '@/lib/api-client';
...
export default function CampaignsPage() {
  const { campaigns, addCampaign, updateCampaign, removeCampaign, addLog, setRunning, accounts, twitterCredentials } = useStore();
  ...
  const getRunUsername = () => {
    if (selectedAccountId) {
      const acc = accounts.find(a => a.id === selectedAccountId);
      return acc?.username || activeUsername;
    }
    return activeUsername;
  };

  const handleRun = async (campaign: Campaign) => {
    const runUser = getRunUsername();
    updateCampaign(campaign.id, { status: 'running' });
    setRunning(true, `Kampanya: ${campaign.name}`);
    addLog(`"${campaign.name}" kampanyası başlatıldı (@${runUser}).`, 'info');

    try {
      const searchResult = await searchTweets(runUser, campaign.target, 10);
      const tweets = searchResult.success && searchResult.data?.tweets ? searchResult.data.tweets : [];
      let completed = 0;

      for (const tweet of tweets) {
        for (const step of campaign.steps) {
          if (step.action === 'like') {
            await likeTweet(runUser, tweet.id);
          } else if (step.action === 'retweet') {
            await retweetTweet(runUser, tweet.id);
          } else if (step.action === 'follow' && tweet.author_username) {
            await followUser(runUser, tweet.author_username);
          } else if (step.action === 'comment' && step.commentText) {
            await postTweet(runUser, step.commentText, tweet.id);
          }
          completed++;
